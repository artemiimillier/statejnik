...
## Картинки, видео и материалы

При первой настройке агент согласует инструменты, бюджет, проверку и доставку изображений. В визуальном режиме — 1–5 смысловых картинок внутри статьи помимо обложки, после черновика и до приёмки; обязательны обработка метаданных и проверка конечных файлов. Нет возможностей — можно явно согласовать текстовый режим, без платной подписки по умолчанию. Для старого проекта эта настройка проводится один раз перед следующей статьёй.

Видео и подарки для CTA берутся только из проверенных источников пользователя и только по задаче; отсутствие допустимо. Files-экспорт переносит локальные assets вместе с Markdown/HTML. Автозагрузка изображений в CMS и видеоплееры не реализованы. Подробно: `skills/statejnik/references/process/semantic-visuals.md`, `references/publishers.md` внутри навыка. Скрипты Python остаются stdlib-only; генератор, обработчик картинок и способ визуального просмотра выбираются отдельно.
